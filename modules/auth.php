...
class authClass extends DATABASE
{
  ...
  function sendMail($email,$userId){
    ...
    $token_ = mt_rand(100000,999999);
          $time = time() + 60; // 300 + 5 phút ( Tính theo giây)
...
// rồi giờ mới căng nè =))
	...
	// Nếu gửi thành công thì insert dữ liệu vào bảng
          if (!$req_check) {
           ...
           $check_time =   time() - (strtotime($req_check['time']) + 60);
           if ($check_time < 0) {
             $ketqua = 'notex';
           }else{
